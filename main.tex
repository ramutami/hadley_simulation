...
\neqns{l}{
  ...
  \qquad\qquad\qquad\qquad - \dfrac{\dfrac{\hat{\Theta}_{jk}+\hat{\Theta}_{j,k+1}}{2}w_{j,k+1}-\dfrac{\hat{\Theta}_{j,k-1}+\hat{\Theta}_{j,k}}{2}w_{j,k}}{\Delta z}-\bracketnormal{\hat{\Theta}_{jk}-\hat{\Theta}_{Ejk}}\tau^{-1}\\[5pt]
...
\begin{algorithm}[H]
  ...
  \begin{algorithmic}[1]
    \Require $\hat{\Theta}^\text{in}[-1:j_\text{max},-1:k_\text{max}]$、$\hat{\Theta}_E[0:j_\text{max}-1,0:k_\text{max}-1]$、$\hat{\Theta}^n[0:j_\text{max}-1,0:k_\text{max}-1]$、$v^\text{in}[0:j_\text{max},0:k_\text{max}-1]$、$w^\text{in}[0:j_\text{max}-1,0:k_\text{max}]$

...
同時に今、$\bracketnormal{d\theta ,dz}^T$の三次元空間上でのベクトル$d\vect{l} = ad\theta\vect{e}_\theta + dz\vect{e}_z$が流れ$\vect{v} = vd\theta + wdz$\footnote{
  $v$はあくまで「南北」流！
}と平行でなければならないので次が成立する。

\neqn{
  ad\theta\vect{e}_\theta + dz\vect{e}_z \parallel vd\theta + wdz\;\leadsto \; \dif{z}{\theta} = \dfrac{aw}{v}
}
...
\neqn{
  \dfrac{1}{a}\pdifop{\theta}\bracketnormal{v\cos\theta} + \pdifop{z}\bracketnormal{w\cos\theta}
}

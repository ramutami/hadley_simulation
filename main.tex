...
\neqns{l}{
  ...
  \qquad\qquad\qquad\qquad -\dfrac{1}{a}\dfrac{\bracketnormal{\pdif{\Phi}{\theta}}_{j-1,k}+\bracketnormal{\pdif{\Phi}{\theta}}_{j-1,k+1}+\bracketnormal{\pdif{\Phi}{\theta}}_{jk}+\bracketnormal{\pdif{\Phi}{\theta}}_{j,k+1}}{4}\\[5pt]
...
\begin{algorithm}[H]
  ...
  \begin{algorithmic}[1]
    \Require $\theta[0:j_\text{max}]$、$u^\text{in}[0:j_\text{max}-1,0:k_\text{max}-1]$、$v^\text{in}[0:j_\text{max},-1:k_\text{max}]$、$w^\text{in}[0:j_\text{max}-1,0:k_\text{max}]$,$\bracketnormal{\frac{\partial\Phi}{\partial\theta}}^\text{in}[0:j_\text{max}-1,0:k_\text{max}]$、$v^n[0:j_\text{max},0:k_\text{max}-1]$
    ...
    \Do{$k=0,k_\text{max}-1$}
      \State $\begin{aligned}[t]
        ...
        &\qquad\qquad\qquad\qquad -\dfrac{1}{a}\dfrac{\bracketnormal{\pdif{\Phi}{\theta}}^\text{in}_{j-1,k}+\bracketnormal{\pdif{\Phi}{\theta}}^\text{in}_{j-1,k+1}+\bracketnormal{\pdif{\Phi}{\theta}}^\text{in}_{jk}+\bracketnormal{\pdif{\Phi}{\theta}}^\text{in}_{j,k+1}}{4}\\[5pt]
...
今回はこの値を$w,\Phi$と同様、$jk$番目のセルの下辺に$\bracketnormal{\frac{\partial\Phi}{\partial\theta}}_{jk}$を格納する形で保持することにする。このとき、あらかじめ$\int _0^z g\frac{\partial\hat{\Theta}}{\partial\theta}dz'$をセル下辺で評価しておくと見通しが良い。今積分はセル中心での値に厚み$\Delta z$をかけて足し合わせることで評価することにしているので、これは次のように計算することができる。
...
\begin{algorithm}[H]
  ...
  \begin{algorithmic}[1]
    \Require $\theta[-1:j_\text{max}]$、$u[0:j_\text{max}-1,0:k_\text{max}-1]$、$v[0:j_\text{max},-1:k_\text{max}]$、$\hat{\Theta}[-1,j_\text{max},0:k_\text{max}-1]$、$w[0:j_\text{max}-1,0:k_\text{max}]$
    \Ensure $\pdif{\Phi}{\theta}[0:j_\text{max}-1,0:k_\text{max}]$
    \Statex
    \Do{$j=0,j_\text{max}-1$}
...
\neqns{l}{
  \text{at}\;\; \theta = \pm\dfrac{\pi}{2} : \;\; \pdif{u}{\theta}=\pdif{\hat{\Theta}}{\theta} = 0
}
...
\neqns{l}{
  \bracketnormal{u,\hat{\Theta}}_{-1,k} = \bracketnormal{u,\hat{\Theta}}_{0,k}\qquad \bracketnormal{u,\hat{\Theta}}_{j_\text{max},k} = \bracketnormal{u,\hat{\Theta}}_{j_\text{max}-1,k} \\[10pt]

...
\vs{0}
　よって確保するメモリは次のようになる。（ただし$\Phi$について実際に保持するのは$\frac{\partial\Phi}{\partial\theta}$であり、$v$と同じ位置に保持することに注意。また、$\Theta_E$についてもメモリを確保する必要があることに注意。）
\neqns{ll}{
  \text{$u,\Theta,\Theta_E$について：}\quad &[0:j_\text{max}-1,0:k_\text{max}-1]\\
  \text{$w$について：}\quad &[0:j_\text{max}-1,0:k_\text{max}]\\
  \text{$v,\frac{\partial\Phi}{\partial\theta}$について：}\quad &[0:j_\text{max},0:k_\text{max}-1]\\
...
\neqns{l}{
  ...
  \qquad\qquad\qquad\qquad - \dfrac{\dfrac{u_{jk}+u_{j,k+1}}{2}w_{j,k+1}-\dfrac{u_{j,k-1}+u_{j,k}}{2}w_{j,k}}{\Delta z} + 2\Omega\sin\theta_j\dfrac{v_{jk}+v_{j+1,k}}{2}\\[5pt]
  \qquad\qquad\qquad\qquad+\dfrac{u_{jk}\dfrac{v_{jk}+v_{j+1,k}}{2}\tan\theta_{j}}{a}+\dfrac{\nu}{\Delta z}\bracketnormal{\dfrac{u_{j,k+1}-u_{jk}}{\Delta z}-\dfrac{u_{jk}-u_{j,k-1}}{\Delta z}}\Bigg\}
}

よって、$u$を更新するルーチンをupdate\_uのように定めればいい。ただし入力（Require）で必要になるメモリの範囲は最終的な出力の範囲から逆算している。

\vs{0.5}

\begin{algorithm}[H]
  \caption*{\textbf{subroutine:} update\_u}
  \begin{algorithmic}[1]
    \Require $\theta[-1:j_\text{max}]$、$u^\text{in}[-1:j_\text{max},-1:k_\text{max}]$、$v^\text{in}[0:j_\text{max},0:k_\text{max}-1]$、$w^\text{in}[0:j_\text{max}-1,0:k_\text{max}]$、$u^n[0:j_\text{max}-1,0:k_\text{max}-1]$
    \Ensure $u^\text{out}[0:j_\text{max}-1,0:k_\text{max}-1]$
    \Statex
    \Do{$j=0,j_\text{max}-1$}
    \Do{$k=0,k_\text{max}-1$}
    \State $\begin{aligned}[t]
      &u_{jk}^\text{out} = u^n_{jk} + \Delta t\Bigg\{-\dfrac{1}{a\cos\theta_j}\dfrac{\dfrac{u^\text{in}_{jk}+u^\text{in}_{j+1,k}}{2}v^\text{in}_{j+1,k}\cos\dfrac{\theta_{j+1}+\theta_{j}}{2}-\dfrac{u^\text{in}_{j-1,k}+u^\text{in}_{j,k}}{2}v^\text{in}_{j,k}\cos\dfrac{\theta_{j}+\theta_{j-1}}{2}}{\Delta \theta} \\[5pt]
      &\qquad\qquad\qquad\qquad - \dfrac{\dfrac{u^\text{in}_{jk}+u^\text{in}_{j,k+1}}{2}w^\text{in}_{j,k+1}-\dfrac{u^\text{in}_{j,k-1}+u^\text{in}_{j,k}}{2}w^\text{in}_{j,k}}{\Delta z} + 2\Omega\sin\theta_j\dfrac{v^\text{in}_{jk}+v^\text{in}_{j+1,k}}{2}\\[5pt]
      &\qquad\qquad\qquad\qquad+\dfrac{u^\text{in}_{jk}\dfrac{v^\text{in}_{jk}+v^\text{in}_{j+1,k}}{2}\tan\theta_{j}}{a}+\dfrac{\nu}{\Delta z}\bracketnormal{\dfrac{u^\text{in}_{j,k+1}-u^\text{in}_{jk}}{\Delta z}-\dfrac{u^\text{in}_{jk}-u^\text{in}_{j,k-1}}{\Delta z}}\Bigg\}
    \end{aligned}$
    \EndDo
    \EndDo
  \end{algorithmic}
...
\subsection{\texorpdfstring{$v$の更新}{vの計算}}

　$v$の更新式は次のようになる。
\neqn{
  v^\text{out} = v^n + \Delta t\curlybracket{-\dfrac{1}{a\cos\theta}\pdifop{\theta}\bracketnormal{v^2\cos\theta}-\pdifop{z}\bracketnormal{wv}-fu-\dfrac{u^2\tan\theta}{a} -\dfrac{1}{a}\pdif{\Phi}{\theta} + \pdifop{z}\bracketnormal{\nu\pdif{v}{z}}}
}

ただし$v^\text{out} = v^{n+1}\;\text{or}\;v^*$であり、右辺の$u,v,w,\frac{\partial \Phi}{\partial \theta}$には$u^n,v^n,w^n,\bracketnormal{\frac{\partial \Phi}{\partial \theta}}^n$ないし$u^*,v^*,w^*,\bracketnormal{\frac{\partial \Phi}{\partial \theta}}^*$が入る。その上で、上式をセル左辺で評価すると次のようになる。
\neqns{l}{
  v_{jk}^\text{out} = v_{jk}^n + \Delta t\Bigg\{-\dfrac{1}{a\cos\dfrac{\theta_{j-1}+\theta_{j}}{2}}\dfrac{\bracketnormal{\dfrac{v_{jk}+v_{j+1,k}}{2}}^2\cos\theta_j - \bracketnormal{\dfrac{v_{j-1,k}+v_{jk}}{2}}^2\cos\theta_{j-1} }{\Delta\theta}\\[5pt]
  \qquad\qquad\qquad\qquad - \dfrac{\dfrac{w_{j-1,k+1}+w_{j,k+1}}{2}\dfrac{v_{j,k+1}+v_{jk}}{2}-\dfrac{w_{j-1,k}+w_{jk}}{2}\dfrac{v_{jk}+v_{j,k-1}}{2}}{\Delta z}\\[5pt]
  \qquad\qquad\qquad\qquad-2\Omega\bracketnormal{\sin\dfrac{\theta_{j-1}+\theta_{j}}{2}}\dfrac{u_{jk}+u_{j-1,k}}{2}-\dfrac{\bracketnormal{\dfrac{u_{jk}+u_{j-1,k}}{2}}^2\tan\dfrac{\theta_{j-1}+\theta_{j}}{2}}{a}\\[5pt]
  \qquad\qquad\qquad\qquad -\dfrac{1}{a}\bracketnormal{\pdif{\Phi}{\theta}}_{jk}\\[5pt]
  \qquad\qquad\qquad\qquad +\dfrac{\nu}{\Delta z}\bracketnormal{\dfrac{v_{j,k+1}-v_{jk}}{\Delta z} - \dfrac{v_{jk}-v_{j,k-1}}{\Delta z}}\Bigg\}
}

ただし、極（すなわち$j=0$と$j=j_\text{max}$）では上式において$\cos\frac{\theta_{j-1}+\theta_{j}}{2}=0$となり式が特異となってしまうため、この点においては$v^\text{out}_{jk}=0$と定めることにする。\footnote{
  直感的には、極を横切る南北流（反対の半球の子午面からまわってくる流れ）は$0$であって欲しいので。
}よって、$v$を更新するルーチンをupdate\_vのように定めればいい。

\vs{0.5}

\begin{algorithm}[H]
  \caption*{\textbf{subroutine:} update\_v}
  \begin{algorithmic}[1]
    \Require $\theta[0:j_\text{max}]$、$u^\text{in}[0:j_\text{max}-1,0:k_\text{max}-1]$、$v^\text{in}[0:j_\text{max},-1:k_\text{max}]$、$w^\text{in}[0:j_\text{max}-1,0:k_\text{max}]$,$\bracketnormal{\frac{\partial\Phi}{\partial\theta}}^\text{in}[1:j_\text{max}-1,0:k_\text{max}-1]$、$v^n[0:j_\text{max},0:k_\text{max}-1]$
    \Ensure $v^\text{out}[0:j_\text{max},0:k_\text{max}-1]$
    \Statex
    \Do{$j=1,j_\text{max}-1$}
    \Do{$k=0,k_\text{max}-1$}
      \State $\begin{aligned}[t]
        &v_{jk}^\text{out} = v_{jk}^n + \Delta t\Bigg\{-\dfrac{1}{a\cos\dfrac{\theta_{j-1}+\theta_{j}}{2}}\dfrac{\bracketnormal{\dfrac{v^\text{in}_{jk}+v^\text{in}_{j+1,k}}{2}}^2\cos\theta_j - \bracketnormal{\dfrac{v^\text{in}_{j-1,k}+v^\text{in}_{jk}}{2}}^2\cos\theta_{j-1} }{\Delta\theta}\\[5pt]
        &\qquad\qquad\qquad\qquad - \dfrac{\dfrac{w^\text{in}_{j-1,k+1}+w^\text{in}_{j,k+1}}{2}\dfrac{v^\text{in}_{j,k+1}+v^\text{in}_{jk}}{2}-\dfrac{w^\text{in}_{j-1,k}+w^\text{in}_{jk}}{2}\dfrac{v^\text{in}_{jk}+v^\text{in}_{j,k-1}}{2}}{\Delta z}\\[5pt]
        &\qquad\qquad\qquad\qquad-2\Omega\bracketnormal{\sin\dfrac{\theta_{j-1}+\theta_{j}}{2}}\dfrac{u^\text{in}_{jk}+u^\text{in}_{j-1,k}}{2}-\dfrac{\bracketnormal{\dfrac{u^\text{in}_{jk}+u^\text{in}_{j-1,k}}{2}}^2\tan\dfrac{\theta_{j-1}+\theta_{j}}{2}}{a}\\[5pt]
        &\qquad\qquad\qquad\qquad -\dfrac{1}{a}\bracketnormal{\pdif{\Phi}{\theta}}_{jk}\\[5pt]
        &\qquad\qquad\qquad\qquad +\dfrac{\nu}{\Delta z}\bracketnormal{\dfrac{v^\text{in}_{j,k+1}-v^\text{in}_{jk}}{\Delta z} - \dfrac{v^\text{in}_{jk}-v^\text{in}_{j,k-1}}{\Delta z}}\Bigg\}
      \end{aligned}$
    \EndDo
    \EndDo
    \State \Comment{極における例外処理}
    \Do{$k=0,k_\text{max}-1$}
    \State $v^\text{out}_{0k}=0$
    \State $v^\text{out}_{j_\text{max},k}=0$
    \EndDo

  \end{algorithmic}
\end{algorithm}

\subsection{\texorpdfstring{$\hat{\Theta}$の計算}{温位の計算}}

　$\hat{\Theta}$の更新式は次のようになる。

\neqn{
  \hat{\Theta}^\text{out} = \hat{\Theta}^n + \Delta t\curlybracket{-\dfrac{1}{a\cos\theta}\pdifop{\theta}\bracketnormal{v\hat{\Theta}\cos\theta}-\pdifop{z}\bracketnormal{w\hat{\Theta}} - \bracketnormal{\hat{\Theta} - \hat{\Theta}_E}\tau^{-1} + \pdifop{z}\bracketnormal{\nu\pdif{\hat{\Theta}}{z}}}
}

ただし$\hat{\Theta}^\text{out} = \hat{\Theta}^{n+1}\;\text{or}\;\hat{\Theta}^*$であり、右辺の$v,w,\hat{\Theta}$には$v^n,w^n,\hat{\Theta}^n$ないし$v^*,w^*,\hat{\Theta}^*$が入る。その上で、上式をセル中心で評価すると次のようになる。
\neqns{l}{
  \hat{\Theta}^\text{out}_{jk} = \hat{\Theta}^n_{jk} + \Delta t\Bigg\{-\dfrac{1}{a\cos\theta_j}\dfrac{\dfrac{\hat{\Theta}_{jk}+\hat{\Theta}_{j+1,k}}{2}v_{j+1,k}\cos\dfrac{\theta_{j+1}+\theta_{j}}{2}-\dfrac{\hat{\Theta}_{j-1,k}+\hat{\Theta}_{j,k}}{2}v_{j,k}\cos\dfrac{\theta_{j}+\theta_{j-1}}{2}}{\Delta \theta}\\[5pt]
  \qquad\qquad\qquad\qquad - \dfrac{\dfrac{\hat{\Theta}_{jk}+\hat{\Theta}_{j,k+1}}{2}w_{j,k+1}-\dfrac{\hat{\Theta}_{j,k-1}+\hat{\Theta}_{j,k}}{2}w_{j,k}}{\Delta z}-\bracketnormal{\hat{\Theta}_{jk}-\hat{\Theta}_{E,jk}}\tau^{-1}\\[5pt]
  \qquad\qquad\qquad\qquad + \dfrac{\nu}{\Delta z}\bracketnormal{\dfrac{\hat{\Theta}_{j,k+1}-\hat{\Theta}_{jk}}{\Delta z}-\dfrac{\hat{\Theta}_{jk}-\hat{\Theta}_{j,k-1}}{\Delta z}}\Bigg\}
}

よって、$\hat{\Theta}$を更新するルーチンをupdate\_potential\_temperatureのように定めればいい。
...
\begin{algorithm}[H]
  \caption*{\textbf{subroutine:} update\_potential\_temperature}
  \begin{algorithmic}[1]
    \Require $\theta[-1:j_\text{max}]$、$\hat{\Theta}^\text{in}[-1:j_\text{max},-1:k_\text{max}]$、$\hat{\Theta}_E[0:j_\text{max}-1,0:k_\text{max}-1]$、$\hat{\Theta}^n[0:j_\text{max}-1,0:k_\text{max}-1]$、$v^\text{in}[0:j_\text{max},0:k_\text{max}-1]$、$w^\text{in}[0:j_\text{max}-1,0:k_\text{max}]$
    \Ensure $\hat{\Theta}^\text{out}[0:j_\text{max}-1,0:k_\text{max}-1]$
    \Statex
    \Do{$j=0,j_\text{max}-1$}
    \Do{$k=0,k_\text{max}-1$}
    \State $\begin{aligned}[t]
      &\hat{\Theta}^\text{out}_{jk} = \hat{\Theta}^n_{jk} + \Delta t\Bigg\{-\dfrac{1}{a\cos\theta_j}\dfrac{\dfrac{\hat{\Theta}^\text{in}_{jk}+\hat{\Theta}^\text{in}_{j+1,k}}{2}v^\text{in}_{j+1,k}\cos\dfrac{\theta_{j+1}+\theta_{j}}{2}-\dfrac{\hat{\Theta}^\text{in}_{j-1,k}+\hat{\Theta}^\text{in}_{j,k}}{2}v^\text{in}_{j,k}\cos\dfrac{\theta_{j}+\theta_{j-1}}{2}}{\Delta \theta}\\[5pt]
      &\qquad\qquad\qquad\qquad - \dfrac{\dfrac{\hat{\Theta}^\text{in}_{jk}+\hat{\Theta}^\text{in}_{j,k+1}}{2}w^\text{in}_{j,k+1}-\dfrac{\hat{\Theta}^\text{in}_{j,k-1}+\hat{\Theta}^\text{in}_{j,k}}{2}w^\text{in}_{j,k}}{\Delta z}-\bracketnormal{\hat{\Theta}^\text{in}_{jk}-\hat{\Theta}_{E,jk}}\tau^{-1}\\[5pt]
      &\qquad\qquad\qquad\qquad + \dfrac{\nu}{\Delta z}\bracketnormal{\dfrac{\hat{\Theta}^\text{in}_{j,k+1}-\hat{\Theta}^\text{in}_{jk}}{\Delta z}-\dfrac{\hat{\Theta}^\text{in}_{jk}-\hat{\Theta}^\text{in}_{j,k-1}}{\Delta z}}\Bigg\}
    \end{aligned}$
    \EndDo
    \EndDo
  \end{algorithmic}
...
\begin{algorithm}[!htbp]
  ...
  \begin{algorithmic}[1]
    \Require $\theta[-1:j_\text{max}]$、$v[0:j_\text{max},0:k_\text{max}-1]$
    \Ensure $w[0:j_\text{max}-1,0:k_\text{max}]$
    \Statex
    \Do{$j=0,j_\text{max}-1$}
    \State $w=0$
    \Do{$k=0,k_\text{max}$}
    \State $w_{jk} = w$
    \State if $k=k_\text{max}$: exit loop
...
{\color{red}ここ、$\pdif{\Phi}{\theta}$ を$v$と同じ場所に置くことで解消されるはず}
...
今回はこの値を$v$と同様、$jk$番目のセルの左辺に$\bracketnormal{\frac{\partial\Phi}{\partial\theta}}_{jk}$を格納する形で保持することにする。このとき、あらかじめ$\int _0^z g\frac{\partial\hat{\Theta}}{\partial\theta}dz'$を評価しておくと見通しが良い。今積分はセル中心での値に厚み$\Delta z$をかけて足し合わせることで評価することにしているので、これは次のように計算することができる。

\neqn{
  \bracketnormal{\dint _0^z g\frac{\partial\hat{\Theta}}{\partial\theta}dz'}_{jk} = \dsum_{k'=0}^{k-1}\bracketnormal{\Delta z g\dfrac{\hat{\Theta}_{j,k'}-\hat{\Theta}_{j-1,k'}}{\Delta \theta}} + \dfrac{1}{2}\Delta z g\dfrac{\hat{\Theta}_{j,k}-\hat{\Theta}_{j-1,k}}{\Delta \theta}
}

ただし今$\frac{\partial\hat{\Theta}}{\partial \theta}$は$v$の位置で評価することに注意。
\vs{0}
...
\neqns{l}{
  ...
  \qquad = \dsum_{k=0}^{k_\text{max}-1}\Bigg[\Delta z\Bigg\{-\dfrac{1}{a\cos\dfrac{\theta_{j-1}+\theta_{j}}{2}}\dfrac{\bracketnormal{\dfrac{v_{jk}+v_{j+1,k}}{2}}^2\cos\theta_j - \bracketnormal{\dfrac{v_{j-1,k}+v_{jk}}{2}}^2\cos\theta_{j-1} }{\Delta\theta}\\[5pt]
  \qquad\qquad\qquad\qquad - \dfrac{\dfrac{w_{j-1,k+1}+w_{j,k+1}}{2}\dfrac{v_{j,k+1}+v_{jk}}{2}-\dfrac{w_{j-1,k}+w_{jk}}{2}\dfrac{v_{jk}+v_{j,k-1}}{2}}{\Delta z}\\[5pt]
  \qquad\qquad\qquad\qquad-2\Omega\bracketnormal{\sin\dfrac{\theta_{j-1}+\theta_{j}}{2}}\dfrac{u_{jk}+u_{j-1,k}}{2}-\dfrac{\bracketnormal{\dfrac{u_{jk}+u_{j-1,k}}{2}}^2\tan\dfrac{\theta_{j-1}+\theta_{j}}{2}}{a}\\[5pt]
  \qquad\qquad\qquad\qquad +\dfrac{\nu}{\Delta z}\bracketnormal{\dfrac{v_{j,k+1}-v_{jk}}{\Delta z} - \dfrac{v_{jk}-v_{j,k-1}}{\Delta z}}\Bigg\}\Bigg]
}
...
よってこれらを用いて$\frac{\partial\Phi}{\partial\theta}$を計算するルーチンをcalculate\_partial\_Phiのように定めればいい。ただし入力変数に必要なメモリの範囲は、予報変数$u,v,\hat{\Theta}$の計算に必要な$w$のメモリ分から逆算している。
...
\begin{algorithm}[H]
  ...
  \begin{algorithmic}[1]
    \Require $\theta[0:j_\text{max}-1]$、$u[0:j_\text{max}-1,0:k_\text{max}-1]$、$v[0:j_\text{max},-1:k_\text{max}]$、$\hat{\Theta}[0:j_\text{max}-1,0:k_\text{max}-1]$、$w[0:j_\text{max}-1,0:k_\text{max}]$
    \Ensure $\pdif{\Phi}{\theta}[1:j_\text{max}-1,0:k_\text{max}-1]$
    \Statex
    \Do{$j=1,j_\text{max}-1$}
    \State \Comment{温位の積分を評価する}
    \State $\text{integral} = 0$
    \Do{$k=0,k_\text{max}-1$}
    \State $\bracketnormal{\dint_0^z g\pdif{\hat{\Theta}}{\theta}dz'}_{jk} = \text{integral} + \dfrac{1}{2}\Delta z g\dfrac{\hat{\Theta}_{j,k}-\hat{\Theta}_{j-1,k}}{\Delta \theta}$
    \State $\text{integral}=\text{integral} + \Delta z g\dfrac{\hat{\Theta}_{j,k}-\hat{\Theta}_{j-1,k}}{\Delta \theta}$
    \EndDo
    \State \Comment{第三項の積分の評価}
    \State $\text{term3}=0$
    \Do{$k=0,k_\text{max}-1$}
    \State $\text{term3} = \text{term3} + \Delta z\bracketnormal{\dint_0^z g\pdif{\hat{\Theta}}{\theta}dz'}_{jk} $
    \EndDo
    ...
    \Do{$k=0,k_\text{max}-1$}
    \State $\begin{aligned}[t]
    &\text{term1} = \text{term1} +\Bigg[\Delta z\Bigg\{-\dfrac{1}{a\cos\dfrac{\theta_{j-1}+\theta_{j}}{2}}\dfrac{\bracketnormal{\dfrac{v_{jk}+v_{j+1,k}}{2}}^2\cos\theta_j - \bracketnormal{\dfrac{v_{j-1,k}+v_{jk}}{2}}^2\cos\theta_{j-1} }{\Delta\theta}\\[5pt]
    &\qquad\qquad\qquad\qquad - \dfrac{\dfrac{w_{j-1,k+1}+w_{j,k+1}}{2}\dfrac{v_{j,k+1}+v_{jk}}{2}-\dfrac{w_{j-1,k}+w_{jk}}{2}\dfrac{v_{jk}+v_{j,k-1}}{2}}{\Delta z}\\[5pt]
    &\qquad\qquad\qquad\qquad-2\Omega\bracketnormal{\sin\dfrac{\theta_{j-1}+\theta_{j}}{2}}\dfrac{u_{jk}+u_{j-1,k}}{2}-\dfrac{\bracketnormal{\dfrac{u_{jk}+u_{j-1,k}}{2}}^2\tan\dfrac{\theta_{j-1}+\theta_{j}}{2}}{a}\\[5pt]
    &\qquad\qquad\qquad\qquad +\dfrac{\nu}{\Delta z}\bracketnormal{\dfrac{v_{j,k+1}-v_{jk}}{\Delta z} - \dfrac{v_{jk}-v_{j,k-1}}{\Delta z}}\Bigg\}\Bigg]
    \end{aligned}$
    ...
    \State \Comment{最終的に求めたいPhiの偏微分の評価}
    \Do{$k=0,k_\text{max}-1$}
...
\subsection{\texorpdfstring{$\hat{\Theta}_E$の計算}{基準温位の計算}}
{\color{red}あとで書く！}

\subsection{メモリ確保・ダミーセル・境界条件}

　これまでの計算から、計算に実質的に必要なメモリの範囲が次のようであるとわかる。
\neqns{ll}{
  \text{$u,\hat{\Theta}$について：}\quad &[-1:j_\text{max},-1:k_\text{max}]\\
  \text{$v$について：}\quad &[0:j_\text{max},-1:k_\text{max}]\\
  \text{$w$について：}\quad &[0:j_\text{max}-1,0:k_\text{max}]\\
  \text{$\frac{\partial\Phi}{\partial\theta}$について：}\quad &[1:j_\text{max}-1,0:k_\text{max}-1]
}

しかし、予報変数$u,v,\hat{\Theta}$の更新においては次の範囲の計算しか行っていない。\footnote{
  診断変数については大丈夫。
}
\neqns{ll}{
  \text{$u,\hat{\Theta}$について：}\quad &[0:j_\text{max}-1,0:k_\text{max}-1]\\
  \text{$v$について：}\quad &[0:j_\text{max},0:k_\text{max}-1]
}

すなわち$\theta,z$方向にそれぞれ一マス分のダミーセルを用意する必要があり、その上で$u,\hat{\Theta}$については$j=-1,j=j_\text{max}$および$k=-1,k=k_\text{max}$の値を、$v$については$k=-1,k=k_\text{max}$の値を、それぞれの更新が終わった後に境界条件に従って更新する必要がある。（\reffig{5}参照。）

\begin{figure}[!htbp]
  \centering
  \includegraphics[width=0.75\textwidth]{./teximage/dummycell.png}
  \caption{ダミーセルを用いた計算の流れ}
  \label{fig:5}
\end{figure}
...
　特に今境界条件は
\neqns{ll}{
  & \text{at}\;\; z=H : \;\; w=0,\;\; \pdif{u}{z}=\pdif{v}{z}=\pdif{\hat{\Theta}}{z} = 0\\[5pt]
  & \text{at}\;\; z=0 : \;\; \; w=0,\;\;\;\pdif{\hat{\Theta}}{z}=0,\;\;\nu\pdif{u}{z} = Cu,\;\;\nu\pdif{v}{z} = Cv\\
}
...
\neqns{l}{
  \text{at}\;\; \theta = \pm\dfrac{\pi}{2} : \;\;  u=\pdif{\hat{\Theta}}{\theta} = 0
}

となっていることが期待される。\footnote{
  考えている子午面の反対の半球の子午面において対称性が成立するならこうだろうな、という考えのもと定めた。
}\footnote{
  色々やってみたところ、なんか$u=0\;,\text{at}\; \theta=\pm\pi/2$の方が結果が安定してるような気がするなあ...うーん。
}\footnote{
  $v$については$\theta = \pm \pi/2$で$v=0$としている。（update\_vのサブルーチン内でそのように計算している。）これも一種の境界条件ではある。
}よってダミーセルにおける値は次のように計算されなければならない。

\neqns{l}{
  u_{-1,k}=-u_{0,k},\qquad
  u_{j_\text{max},k}=-u_{j_\text{max}-1,k}
  \\[8pt]
  \hat{\Theta}_{-1,k}=\hat{\Theta}_{0,k},\qquad
  \hat{\Theta}_{j_\text{max},k}=\hat{\Theta}_{j_\text{max}-1,k}\\[10pt]
  \bracketnormal{u,v,\hat{\Theta}}_{j,k_\text{max}} = \bracketnormal{u,v,\hat{\Theta}}_{j,k_\text{max}-1}\\[10pt]
  \hat{\Theta}_{j,-1}=\hat{\Theta}_{j,0}\qquad\bracketnormal{u,v}_{j,-1}=\dfrac{\frac{\nu}{\Delta z}-\frac{C}{2}}{\frac{\nu}{\Delta z}+\frac{C}{2}}\bracketnormal{u,v}_{j,0}\labeqn{63}
}

ただし\refeqn{63}右式は$z=0$での境界条件を離散化することで得られる下記の式を整理することで得られる。

\neqn{
  \text{at}\; z=0\;\; :\nu\pdif{\bracketnormal{u,v}}{z} = C\times \bracketnormal{u,v} \;\leadsto \; \nu\dfrac{\bracketnormal{u,v}_{j,0}-\bracketnormal{u,v}_{j,-1}}{\Delta z} = C\dfrac{\bracketnormal{u,v}_{j,0}+\bracketnormal{u,v}_{j,-1}}{2}
}
...
\begin{algorithm}[H]
  \caption*{\textbf{subroutine:} update\_boundary\_u}
  \begin{algorithmic}[1]
    \Require $u[-1:j_\text{max},-1:k_\text{max}]$
    \Ensure $u[-1:j_\text{max},-1:k_\text{max}]$
    \Statex
    \State \Comment{極境界}
    \Do{$k=0,k_\text{max}-1$}
    \State $u_{-1,k} = u_{0,k}$
    \State $u_{j_\text{max},k} = u_{j_\text{max}-1,k}$
    \EndDo
    \State \Comment{上下境界}
    \Do{$j=-1,j_\text{max}$}
    \State $u_{j,k_\text{max}} = u_{j,k_\text{max}-1}$
    \State $u_{j,-1} = \dfrac{\frac{\nu}{\Delta z}-\frac{C}{2}}{\frac{\nu}{\Delta z}+\frac{C}{2}}u_{j,0}$
    \EndDo
  \end{algorithmic}
\end{algorithm}
...
\begin{algorithm}[H]
  \caption*{\textbf{subroutine:} update\_boundary\_potential\_temp}
  \begin{algorithmic}[1]
    \Require $\hat{\Theta}[-1:j_\text{max},-1:k_\text{max}]$
    \Ensure $\hat{\Theta}[-1:j_\text{max},-1:k_\text{max}]$
    \Statex
    \State \Comment{極境界}
    \Do{$k=0,k_\text{max}-1$}
    \State $\hat{\Theta}_{-1,k} = \hat{\Theta}_{0,k}$
    \State $\hat{\Theta}_{j_\text{max},k} = \hat{\Theta}_{j_\text{max}-1,k}$
    \EndDo
    \State \Comment{上下境界}
    \Do{$j=-1,j_\text{max}$}
    \State $\hat{\Theta}_{j,k_\text{max}} = \hat{\Theta}_{j,k_\text{max}-1}$
    \State $\hat{\Theta}_{j,-1} = \hat{\Theta}_{j,0}$
    \EndDo
  \end{algorithmic}
\end{algorithm}
...
\begin{algorithm}[H]
  \caption*{\textbf{subroutine:} update\_boundary\_v}
  \begin{algorithmic}[1]
    \Require $v[0:j_\text{max},-1:k_\text{max}]$
    \Ensure $v[0:j_\text{max},-1:k_\text{max}]$
    \Statex
    \State \Comment{上下境界}
    \Do{$j=0,j_\text{max}$}
    \State $v_{j,k_\text{max}} = v_{j,k_\text{max}-1}$
    \State $v_{j,-1} = \dfrac{\frac{\nu}{\Delta z}-\frac{C}{2}}{\frac{\nu}{\Delta z}+\frac{C}{2}}v_{j,0}$
    \EndDo
  \end{algorithmic}
\end{algorithm}

\section{初期条件・パラメタの設定}
{\color{red}あとで書く！}

\section{出力}
　ハドレー循環を観察するため、流線関数を出力する。

\subsection{子午面流線関数}
今、子午面での流線関数を$\Psi\bracketnormal{\theta,z}$とおく。この時、流線関数の定義より$\Psi$の等値線は流れの向きに沿っていなければならない。今、$\Psi$の等値線はその定義から次を満たす。

\neqn{
  d\Psi = \pdif{\Psi}{\theta}d\theta + \pdif{\Psi}{z}dz = 0 \; \leadsto \; \dif{z}{\theta} = -\dfrac{\pdif{\Psi}{\theta}}{\pdif{\Psi}{z}}
}

同時に今、$\bracketnormal{d\theta ,dz}^T$の三次元空間上でのベクトル$d\vect{l} = ad\theta\vect{e}_\theta + dz\vect{e}_z$が流れ$\vect{v} = v\vect{e}_\theta + w\vect{e}_z$\footnote{
  $v$はあくまで「南北」流！
}と平行でなければならないので次が成立する。

\neqn{
  ad\theta\vect{e}_\theta + dz\vect{e}_z \parallel v\vect{e}_\theta + w\vect{e}_z\;\leadsto \; \dif{z}{\theta} = \dfrac{aw}{v}
}

よって二つの条件を見比べれば、流線関数$\Psi$について次が成立していなければならない。

\neqn{
  v\pdif{\Psi}{\theta} + aw\pdif{\Psi}{z} = 0
}

よって流線関数は$\frac{\partial \Psi}{\partial z} = -Mv,\; \frac{\partial \Psi}{\partial \theta} = aMw$のような形でなくてはならない。さらに、数学的な要請から$\frac{\partial^2 \Psi}{\partial z\partial\theta} = \frac{\partial^2 \Psi}{\partial \theta\partial z}$でなければならないので、次が成立しなければならない。

\neqn{
  \pdifop{\theta}\bracketnormal{-Mv} = \pdifop{z}\bracketnormal{aMw}\;\leftrightarrow \; \dfrac{1}{a}\pdifop{\theta}\bracketnormal{Mv} + \pdifop{z}\bracketnormal{Mw} = 0
}

そこで、非発散条件を思い出してみると、（整理することで）以下のようになる。

\neqn{
  \dfrac{1}{a}\pdifop{\theta}\bracketnormal{v\cos\theta} + \pdifop{z}\bracketnormal{w\cos\theta}=0
}

よって上二式を見比べると$M=\cos\theta$となっていれば良いだろうとわかる。よって流線関数は次を満たす。

\neqn{
  \pdif{\Psi}{z} = -v\cos\theta\quad \pdif{\Psi}{\theta} = aw\cos\theta
}

\subsection{流線関数の計算}
　今、$\pdif{\Psi}{\theta} = aw\cos\theta$が成立するので、$z=0$で$w=0$という境界条件と合わせると
\neqn{
  \left.\pdif{\Psi}{\theta}\right|_{z=0} = 0 \;\leadsto \; \Psi\bracketnormal{\theta,0} = \text{const.} = C(t)
}

となっており、地表面で流線関数の値は一定となっていることがわかる。その上で、今流線関数についてはその等高線のみに興味があり定数の差に関心はないので、任意の時刻$t$で$C(t)=0$としてしまって良い。よって、任意の点における流線関数が

\neqns{lcl}{
  \Psi\bracketnormal{\theta,z} &\neqnsequal& \Psi\bracketnormal{\theta,0} + \dint_0^z \pdif{\Psi}{z}dz'\\
  &\neqnsequal & \dint_0^z -v\cos\theta dz'
}

と計算できる。よっていま、$\Psi_{jk}$を$w,\Phi$と同様に$jk$番目のセルの下辺に保持することにすれば、$\Psi_{jk}$の計算スキームは次のようになる。

\neqn{
  \Psi_{jk} = \dsum_{k'=0}^{k-1} \bracketnormal{-\dfrac{v_{jk'}+v_{j+1,k'}}{2}\cos\theta_j\Delta z}
}

よって次のようなルーチンを実装すれば良い。
\vs{0.5}

\begin{algorithm}[H]
  \caption*{\textbf{subroutine:} calculate\_stream\_function}
  \begin{algorithmic}[1]
    \Require $v[0:j_\text{max},0:k_\text{max}-1]$
    \Ensure $\Psi[0:j_\text{max}-1,0:k_\text{max}]$
    \Statex
    \Do{$j=0,j_\text{max}-1$}
    \State $\text{val}=0$
    \Do{$k=0,k_\text{max}$}
    \State $\Psi_{jk}=\text{val}$
    \State if $k=k_\text{max}$ exit
    \State $\text{val}=\text{val}+\bracketnormal{-\dfrac{v_{jk}+v_{j+1,k}}{2}\cos\theta_j\Delta z}$
    \EndDo
    \EndDo
